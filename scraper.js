...
request(url, function (err, res, html) { 
  if (!err && res.statusCode == 200) { // If we get anything but a successful 200 HTTP code, log the error and stop the program
    var $ = cheerio.load(html);
      ...
      for (var i = 0; i < linksSeen.length; i++) { 
        if (linksSeen[i].indexOf("?id=") > 0 ) { 
          shirtsToScrape.push(linksSeen[i]);
        } else {
          request(linksSeen[i], function(err, res, html) { 
            if (!err && res.statusCode == 200) { 
              var $ = cheerio.load(html);
                ...
                $("a[href*='shirt.php?id=']").each(function() {
                  ...
                  for (var s = 0; s < shirtsToScrape.length; s++) { 
                  request(shirtsToScrape[s], function (err, res, html) { 
                    if (!err && res.statusCode == 200) { 
                      var $ = cheerio.load(html);
                      // Grab all shirt data
                      ...
                      shirts.URL = res.request.uri.href;
